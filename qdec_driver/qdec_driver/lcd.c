...
#if LCD_IO_MODE
#if LCD_LINES==1
#define LCD_FUNCTION_DEFAULT    LCD_FUNCTION                 _4BIT_1LINE 
#else
...
static uint8_t lcd_read(uint8_t rs) 
{
      ...
      && ( LCD_DATA0_PIN == 0 )&& (LCD_DATA1_PIN == 1) && (LCD_DATA2_PIN == 2) && (LCD_DATA3_PIN == 3) )
    {
        ...
        lcd_e_high();
        lcd_e_delay();        
        data = PIN(LCD_DATA0_PORT.OUT) << 4;     /* read high nibble first */
        lcd_e_low();
        ...
        lcd_e_high();
        lcd_e_delay();
        data |= PIN(LCD_DATA0_PORT.OUT)&0x0F;    /* read low nibble        */
        lcd_e_low();
    }
    else
    {
        ...
        lcd_e_delay();        
        data = 0;
        if ( PIN(LCD_DATA0_PORT.OUT) & _BV(LCD_DATA0_PIN) ) data |= 0x10;
        if ( PIN(LCD_DATA1_PORT.OUT) & _BV(LCD_DATA1_PIN) ) data |= 0x20;
        if ( PIN(LCD_DATA2_PORT.OUT) & _BV(LCD_DATA2_PIN) ) data |= 0x40;
        if ( PIN(LCD_DATA3_PORT.OUT) & _BV(LCD_DATA3_PIN) ) data |= 0x80;
        lcd_e_low();
        ...
        lcd_e_high();
        lcd_e_delay();
        if ( PIN(LCD_DATA0_PORT.OUT) & _BV(LCD_DATA0_PIN) ) data |= 0x01;
        if ( PIN(LCD_DATA1_PORT.OUT) & _BV(LCD_DATA1_PIN) ) data |= 0x02;
        if ( PIN(LCD_DATA2_PORT.OUT) & _BV(LCD_DATA2_PIN) ) data |= 0x04;
        if ( PIN(LCD_DATA3_PORT.OUT) & _BV(LCD_DATA3_PIN) ) data |= 0x08;        
        lcd_e_low();
    }
    return data;
}
